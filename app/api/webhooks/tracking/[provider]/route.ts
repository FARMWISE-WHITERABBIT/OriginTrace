...
interface TrackingAdapter {
  ...
  /** Map the provider's payload to (referenceId, normalized events). */
  parse(rawBody: string): { referenceId: string; events: NormalizedShippingEvent[] };
}
...
const adapters: Record<string, TrackingAdapter> = {
  ...
  // terminal49 / vizion: add adapters here after the pilot — each maps the
  // provider's webhook body onto NormalizedShippingEvent and verifies the
...
export async function POST(
  ...
  let events: NormalizedShippingEvent[];
  try {
    ({ referenceId, events } = adapter.parse(rawBody));
  } catch {
    return NextResponse.json({ error: 'Invalid payload' }, { status: 400 });
  }

  const summary = { stored: 0, duplicates: 0, no_subscription: 0, outcomes: [] as string[] };
