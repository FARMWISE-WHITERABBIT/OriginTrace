 ...
 * List compliance profiles (EUDR, FSMA204, Lacey Act, etc.).
 *
 * Auth: API key with 'read' scope (Authorization: Bearer <key>)
 * Query params: limit (max 500), offset, framework, status
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const regulation_framework = searchParams.get('framework');
    const status = searchParams.get('status');

    let query = supabase
      .from('compliance_profiles')
      .select('id, regulation_framework, overall_score, status, last_assessed_at, created_at', { count: 'exact' })
      .eq('org_id', auth.orgId)
      .order('created_at', { ascending: false })
      .range(offset, offset + limit - 1);

    if (regulation_framework) query = (query as any).eq('regulation_framework', regulation_framework);
    if (status) query = (query as any).eq('status', status);

    const { data, error, count } = await query;
    if (error) {
      console.error('[v1/compliance-profiles]', error.message);
    ...
    console.error('[v1/compliance-profiles]', err?.message);
