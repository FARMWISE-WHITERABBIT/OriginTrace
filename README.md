...
### Onboarding Policy
OriginTrace uses a **restricted onboarding model**. Unlike typical SaaS platforms with open registration, OriginTrace enforces strict manual vetting to ensure platform integrity and regulatory compliance.
- **Exporters & Buyers**: Self-service registration is disabled. New organizations are provisioned by the Superadmin only after signing service contracts and completing our manual verification process.
- **Farmers**: Onboarded via invite-only links (SMS) generated by their parent organization. This is the only semi-automated onboarding flow allowed, and it is restricted to pre-vetted suppliers.
