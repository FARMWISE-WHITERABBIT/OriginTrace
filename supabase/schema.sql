...
-- Separate from org-level audit_logs — tracks all superadmin
-- actions: org creation, tier changes, impersonation, feature
-- toggles, user management, billing operations.
-- ============================================================

CREATE TABLE IF NOT EXISTS superadmin_audit_logs (
  id               UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  superadmin_id    UUID NOT NULL REFERENCES auth.users(id) ON DELETE SET NULL,
  action           TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_superadmin_audit_superadmin ON superadmin_audit_logs(superadmin_id);
...
-- Only service role can read/write (superadmin routes use admin client)
CREATE POLICY "superadmin_audit_service_only" ON superadmin_audit_logs
  USING (false);

-- ============================================================
-- BILLING & SUBSCRIPTION MANAGEMENT
-- Superadmin creates payment links per org.
-- Paystack webhook auto-upgrades tier on charge.success.
-- Cron job enforces grace period and downgrades.
-- ============================================================
