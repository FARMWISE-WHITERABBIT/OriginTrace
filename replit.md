...
### Backend
Implemented using Next.js serverless functions (App Router API routes) with TypeScript. Authentication is handled by Supabase Auth (`createServerClient()`). Admin operations use `createAdminClient()`. APIs are RESTful, with an enterprise API layer (`/api/v1/`) supporting API key authentication, scope enforcement, DB-backed rate limiting, and tier gating. RBAC is enforced in middleware.
